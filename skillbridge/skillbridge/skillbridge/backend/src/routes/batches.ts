...
router.post('/', requireAuth, requireRole('TRAINER', 'INSTITUTION'), async (req: Request, res: Response) => {
  try {
    const { name } = req.body
    const user = req.user
...
router.post('/:id/invite', requireAuth, requireRole('TRAINER'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
...
router.post('/:id/join', requireAuth, requireRole('STUDENT'), async (req: Request, res: Response) => {
  try {
...
router.get('/:id/summary', requireAuth, requireRole('INSTITUTION', 'PROGRAMME_MANAGER', 'MONITORING_OFFICER'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params
    ...
    const summary = batch.students.map((bs: { studentId: any; student: { name: any } }) => {
      const studentAttendance = batch.sessions.flatMap((s: { attendance: any[] }) => 
        s.attendance.filter((a: { studentId: any }) => a.studentId === bs.studentId)
      )
      const present = studentAttendance.filter((a: { status: string }) => a.status === 'PRESENT').length
      return {
...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (user.role === 'TRAINER') {
      ...
      batches = trainerBatches.map((tb: { batch: any }) => tb.batch)
    } else if (user.role === 'STUDENT') {
      const studentBatches = await prisma.batchStudent.findMany({
        where: { studentId: user.id },
        include: { batch: true }
      })
      batches = studentBatches.map((sb: { batch: any }) => sb.batch)
